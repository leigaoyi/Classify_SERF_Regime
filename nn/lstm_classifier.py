...
##准备数据集函数，列出模型。
data_shuffle = np.load('data.npy')
labels_shuffle = np.load('label.npy')
...
#model, train_dict = train(model, trainloader, valloader, n_epochs=2, optimizer=optimizer)

# conv_layers = [(4, 1, 1), (4, 1, 1)]
# encoder = CNN1d(conv_layers)
# classifier = LSTM_Attn_Classifier(4, 2, 2)
...
#scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=1e-4, max_lr=1e-1, step_size_up=10)
model, train_dict = train(model, trainloader, valloader, n_epochs=200, optimizer=optimizer)
